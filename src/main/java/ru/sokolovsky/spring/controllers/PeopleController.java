...
public class PeopleController {
    ...
    public String show(@PathVariable("id") int id, Model model, @ModelAttribute("book") Book book) {
        List<Book> personBook = bookDAO.getBookIdByAuthor(id);

        if (!personBook.isEmpty())
            model.addAttribute("getBooks", personBook);
        else
            model.addAttribute("getPerson", personDAO.index());
